...
Rcpp::List rangerCpp(uint treetype, Rcpp::NumericMatrix& input_x, Rcpp::NumericMatrix& input_y,
  ...
  //printf("Are we ever getting here?");
  ...
  try {
    ...
    case TREE_CLASSIFICATION:
      ...
      break;
    case TREE_CLUSTERING:
        //printf("START: Make unique");
        forest = std::make_unique<ForestClassification>();
        //printf("END: Make unique");
      break;
    ...
    if (prediction_mode) {
      ...
      if (treetype == TREE_CLASSIFICATION || treetype == TREE_CLUSTERING) {
      ...
      // Set class weights
      if ((treetype == TREE_CLASSIFICATION || treetype == TREE_CLUSTERING) && !class_weights.empty()) {
        auto& temp = dynamic_cast<ForestClassification&>(*forest);
