...
// Stop early if no split posssible
      ...
      if (data->isOrderedVariable(varID)) {
        ...
        //printf("2");
          ...
          if (q < Q_THRESHOLD) {
            ...
            //findBestSplitValueSmallQ(nodeID, varID, num_classes, class_counts, num_samples_node, best_value, best_varID,
            //    best_decrease);
            findBestSplitValueLargeQSilhouette(nodeID, varID, num_classes, class_counts, num_samples_node, best_value, best_varID,
            best_decrease, best_decrease2);
          } else {
           // printf("4");
            findBestSplitValueLargeQSilhouette(nodeID, varID, num_classes, class_counts, num_samples_node, best_value, best_varID,
            best_decrease, best_decrease2);
            //findBestSplitValueLargeQ(nodeID, varID, num_classes, class_counts, num_samples_node, best_value, best_varID,
            //    best_decrease); // here it goes!!!
          }
        }
      } else {

        //printf("5"); //does not enter this
            
        if(1){//if (splitrule == HELLINGER) {
          findBestSplitValueLargeQSilhouette(nodeID, varID, num_classes, class_counts, num_samples_node, best_value, best_varID,
            best_decrease, best_decrease2);
        }else{
  ...
  if (importance_mode == IMP_GINI || importance_mode == IMP_GINI_CORRECTED) {
    ...
    //printf("Num classes: %d", (int)num_classes);
    
    if(num_classes==41){ // why 41? when no target is given
      //printf(">1");
      addGiniImportance(nodeID, best_varID, best_decrease);
    }else{
      //printf("<1");
      addGiniImportance(nodeID, best_varID, best_decrease2);
    }
  }
...
// ########################################
// Modification for unsupervised RF - Pfeifer
// ########################################

void TreeClassification::findBestSplitValueLargeQSilhouette(size_t nodeID, size_t varID, size_t num_classes,
    const std::vector<size_t>& class_counts, size_t num_samples_node, double& best_value, size_t& best_varID,
    double& best_decrease, double& best_decrease2) {


  // Set counters to 0
  size_t num_unique = data->getNumUniqueDataValues(varID);
  //std::vector<double> values(num_unique); // Pfeifer
  std::vector<double> values(num_samples_node); // Pfeifer
  ...
  for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
    size_t sampleID = sampleIDs[pos];
    size_t index = data->getIndex(sampleID, varID);
    double value = data->getUniqueDataValue(varID, index); // seems to be unordered!
    /*printf("%f", value);
    printf("-%lu", sampleID);
    printf("\n");
    printf(",,");*/
    values[count] = value; //Pfeifer
    count++;
  ...
  */
  ...
  for (size_t i = 0; i < num_unique - 1; ++i) {
    ...
    if (splitrule == HELLINGER) {
      ...
      // Decrease of impurity
      decrease2 = sum_right / (double) n_right + sum_left / (double) n_left;
    }
    ...
    double leftValuesMean=0;
    double rightValuesMean=0;
    int leftCounter=0;
    int rightCounter=0;
    double candidate_value;

    candidate_value = data->getUniqueDataValue(varID, i); //values[i];
    
    // START FINDING THE BEST SPLIT // Pfeifer
    for (int xx = 0; xx<num_samples_node; xx++){
      // find the split
      if(values[xx]<=candidate_value){
        if(values[xx]!=0){
          leftValues = leftValues + values[xx];
          leftCounter++;
        } 
      }else{
        if(values[xx]!=0){
          rightValues = rightValues + values[xx];
          rightCounter++;
        }
      }
    }

    leftValuesMean = leftValues/(double)leftCounter;
    rightValuesMean = rightValues/(double)rightCounter;
    

    /*
    printf("\n");
    printf("%f", leftValuesMean);
    printf("\n");

    printf("\n");
    printf("%f", rightValuesMean);
    printf("\n");
    */

    double my_decrease = fabs(leftValuesMean - rightValuesMean);

    /*printf("##### \n");
    printf("%f", my_decrease);
    printf("##### \n");
    */

    decrease = my_decrease; // Pfeifer

    // PAIRWISE COMPARISON - PFEIFER

    std::vector<double> leftValuesVector(leftCounter);
    std::vector<double> rightValuesVector(rightCounter);
    leftCounter=0;
    rightCounter=0;
    ...
    for (int xx = 0; xx<num_samples_node; xx++){
      ...
      if(values[xx]<=candidate_value){
        if(values[xx]!=0){
          leftValuesVector[leftCounter] =  values[xx];
          leftCounter++;
        } 
      }else{
        if(values[xx]!=0){
          rightValuesVector[rightCounter] = values[xx];
          rightCounter++;
        }
      }
    }
    ...
    for (int xx = 0; xx < (leftCounter-1); xx++){
      for(int yy = xx+1; yy < leftCounter; yy++){
        //diversity1 = diversity1 + fabs(leftValuesVector[xx] - leftValuesVector[yy]);
        diversity1 = diversity1 + (leftValuesVector[xx] - leftValuesVector[yy])*(leftValuesVector[xx] - leftValuesVector[yy]);
        comp1++;
      }
    }
    diversity1 = diversity1/comp1;

    // diversity #2
    double diversity2 = 0;
    double comp2 = 0;
    // Calculate pairwise comparisons
    for (int xx = 0; xx < (rightCounter-1); xx++){
      for(int yy = xx+1; yy < rightCounter; yy++){
        //diversity2 = diversity2 + fabs(rightValuesVector[xx] - rightValuesVector[yy]);
        diversity2 = diversity2 + (rightValuesVector[xx] - rightValuesVector[yy])*(rightValuesVector[xx] - rightValuesVector[yy]);
        comp2++;
      }
    }
    diversity2 = diversity2/comp2;
    ...
    int comp = 0;
    // Calculate pairwise comparisons
    for (int xx = 0; xx < leftCounter; xx++){
      for(int yy = 0; yy < rightCounter; yy++){
        //divergence = divergence + fabs(leftValuesVector[xx] - rightValuesVector[yy]);
        divergence = divergence + (leftValuesVector[xx] - rightValuesVector[yy])*(leftValuesVector[xx] - rightValuesVector[yy]);
        comp++;
      }
    }
    
    divergence = divergence / (double)comp;

    //decrease = divergence;
    decrease = 1 - diversity/divergence; 
...
void TreeClassification::findBestSplitValueUnorderedSilhouette(size_t nodeID, size_t varID, size_t num_classes,
    const std::vector<size_t>& class_counts, size_t num_samples_node, double& best_value, size_t& best_varID,
    double& best_decrease) {
  ...
  for (size_t local_splitID = 1; local_splitID < num_splits / 2; ++local_splitID) {
    ...
    if (splitrule == HELLINGER) {
      ...
      // Decrease of impurity
      // decrease = sum_left / (double) n_left + sum_right / (double) n_right;
      decrease = left_value_my/n_left_my - right_value/n_right; //abs(mean(left) - mean(right))
    }
